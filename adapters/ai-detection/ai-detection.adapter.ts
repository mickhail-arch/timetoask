...
/**
 * AI-детект через Winston AI (внешний сервис).
 * Возвращает human score 0-100 (инвертирован: 100 = человек, 0 = AI).
 * Для совместимости с пайплайном конвертируем в "AI score": 100 - human_score.
 * Таймаут 30 сек. При ошибке — fallback на LLM-детект.
 */
export async function detectAIWinston(
  text: string,
  fallbackModel?: string,
): Promise<AIDetectResult> {
  const apiKey = env.WINSTON_API_KEY;

  if (!apiKey) {
    console.warn('[ai-detection] WINSTON_API_KEY not set, falling back to LLM');
    return fallbackModel ? detectAI(text, fallbackModel) : { score: 0, markers: [], fix_instructions: [] };
  }

  if (text.length < 300) {
    console.warn('[ai-detection] Text too short for Winston (<300 chars), skipping');
    return { score: 0, markers: [], fix_instructions: [] };
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.warn(`[ai-detection] Winston API error ${response.status}: ${errorText}`);
      if (fallbackModel) return detectAI(text, fallbackModel);
      return { score: 0, markers: [], fix_instructions: [] };
    }
    ...
    const data = await response.json() as {
      ...
      length: number;
      score: number;
      sentences: Array<{ text: string; score: number; length: number }>;
      input: string;
      readability_score: number;
      credits_used: number;
      credits_remaining: number;
    ...
    const aiScore = Math.max(0, Math.min(100, Math.round(100 - (data.score ?? 50))));

    const problematicSentences = (data.sentences ?? [])
      .filter(s => s.score < 40)
    ...
    if (problematicSentences.length > 0) {
      markers.push(`${problematicSentences.length} предложений с высокой вероятностью AI (Winston)`);
      for (const s of problematicSentences) {
        const snippet = s.text.length > 80 ? s.text.slice(0, 77) + '...' : s.text;
        markers.push(`[AI ${100 - s.score}%] "${snippet}"`);
        fix_instructions.push(`Переписать: "${snippet}" — добавить конкретику, изменить структуру`);
      }
    }

    if (data.attack_detected?.zero_width_space || data.attack_detected?.homoglyph_attack) {
      markers.push('Обнаружена попытка обхода AI-детекта (спецсимволы)');
    }

    console.info(`[ai-detection] Winston: human_score=${data.score}, ai_score=${aiScore}, readability=${data.readability_score}, credits_used=${data.credits_used}, credits_remaining=${data.credits_remaining}`);
    ...
    console.warn(`[ai-detection] Winston failed: ${message}, falling back to LLM`);
    if (fallbackModel) return detectAI(text, fallbackModel);
    return { score: 0, markers: [], fix_instructions: [] };
  }
}
