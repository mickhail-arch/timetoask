...
Лучше написать на 3-5% больше целевого, чем недобрать.
...
function buildImagesBlock(imageCount: number, brief: BriefData): string {
...
ВАЖНО ПО ОПИСАНИЯМ:
- Описание = 2-3 предложения на русском языке. Не одно слово, не общая фраза.
- Описание привязано к КОНКРЕТНОМУ разделу где стоит маркер.
- Каждое описание УНИКАЛЬНО — разные объекты, ракурсы, композиция.
- Описывай конкретную сцену: что изображено, какие объекты, какое действие, какой ракурс.

Плохо: "Мобильное приложение для покупок"
Хорошо: "Смартфон на деревянном столе с открытым каталогом кроссовок Nike Air Jordan — на экране видна сетка товаров с ценами в юанях и зелёными бейджами проверки подлинности"

Плохо: "Доставка товаров из Китая"  
Хорошо: "Курьерская посылка с китайскими иероглифами и логотипом Dewu на фоне карты маршрута Шанхай-Москва, рядом таможенная декларация и калькулятор"

Контекст разделов для привязки изображений:
${h2Descriptions}

Расстановка:
- Первый маркер — сразу после H1, перед вводным абзацем. Это обложка статьи — самое общее изображение по теме.
- Остальные — равномерно по H2-блокам. Каждое изображение иллюстрирует КОНКРЕТНЫЙ аспект раздела.
...
Без воды и повторения того что уже сказано.`);
  ...
  // 10. Картинки
  if (imageCount > 0) {
    blocks.push(buildImagesBlock(imageCount, brief));
  }
