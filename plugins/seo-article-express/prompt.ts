...
КРИТИЧНО: количество H3 строго ограничено таблицей в ТЗ. Для статьи 4000 символов — максимум 2 H3. Для 6000 — максимум 3. Для 8000 — максимум 4. Каждый лишний H3 раздувает объём. Если в ТЗ указано 2 H3, пиши строго 2 H3 и ни одним больше.`;
}
...
| Блок сравнения (H3 + ul/li) | 4% | ~${tableChars} |
| Личный опыт / кейс | 4% | ~${caseChars} |
| Заключение (H2) | 4% | ~${conclusionChars} |
| FAQ (H2) | 10% | ~${faqChars} |
...
КРИТИЧНО: «Кратко» — это НЕ заголовок. НЕ используй H2 или H3 для слова «Кратко». Только <p><strong>Кратко</strong></p>. Это стилевой элемент, не структурный заголовок.
...
function buildTableBlock(charCount: number, brief: BriefData): string {
  ...
  const comparisonObjects = charCount <= 8000 ? 2 : charCount <= 14000 ? 3 : 4;
  const comparisonCriteria = charCount <= 8000 ? 3 : 4;

  return `=== БЛОК СРАВНЕНИЯ (4% от статьи, ~${tableChars} символов) ===

Один блок сравнения в статье. Размести внутри H2 #${tableAfterH2 + 1} (считая от начала).
Тема сравнения: ${tableTopic}

Количество объектов сравнения: ${comparisonObjects}. Критериев на каждый: ${comparisonCriteria}.
...
function buildCaseBlock(charCount: number, brief: BriefData): string {
  if (charCount <= 5000) {
    const caseChars = brief.case_chars ?? Math.round(charCount * 0.05);
    return `=== ЛИЧНЫЙ ОПЫТ / КЕЙС (5%, ~${caseChars} символов) ===\nКороткий кейс в 2-3 предложения: конкретная ситуация + результат с цифрой. Без подробного разбора. Тема: ${brief.case_topic ?? 'практический опыт по теме статьи'}`;
  }
...
function buildCitationsBlock(charCount: number, brief: BriefData): string {
  const count = brief.citation_count ?? (charCount <= 6000 ? 0 : charCount <= 10000 ? 1 : 2);

  if (count === 0) {
    return `=== ЦИТАТЫ ЭКСПЕРТОВ ===\nДля статьи такого объёма цитаты экспертов не нужны. Не добавляй blockquote с cite.`;
  }
...
function buildCalloutBlock(charCount: number, brief: BriefData): string {
  const count = brief.callout_count ?? (charCount <= 6000 ? 1 : charCount <= 10000 ? 2 : charCount <= 14000 ? 3 : 4);
...
Ниже — образец H2-блока со всеми типами элементов. Это демонстрация структуры и стиля.

<h2>Как работает система аутентификации</h2>
<p>Каждый товар проходит 9 этапов проверки перед отправкой покупателю. Эксперты анализируют материалы, швы, маркировку и упаковку. В 2024 году платформа отклонила более 3% товаров.</p>
<blockquote><p><strong>❗ Важно</strong><br>Проверка занимает 2-3 рабочих дня — учитывайте это при расчёте сроков доставки</p></blockquote>
<h3>Этапы проверки подлинности</h3>
<p>Первый этап — визуальный осмотр упаковки. Специалисты сверяют штрих-коды с базой производителя. Далее товар переходит к экспертам по материалам.</p>
<blockquote><p>За последний год система отклонила 47 000 подделок — это серьёзный показатель для рынка</p><cite>Ван Лэй, технический директор Dewu, TechCrunch 2024</cite></blockquote>
<p>Зачем столько проверок? За последний год 4 из 100 посылок содержали несоответствия.</p>
<h3>Сравнение платформ по проверке</h3>
<h3>Poizon</h3>
<ul><li><strong>Этапов проверки:</strong> 9</li><li><strong>Срок проверки:</strong> 2-3 дня</li><li><strong>Процент отказов:</strong> 3.2%</li></ul>
<h3>StockX</h3>
<ul><li><strong>Этапов проверки:</strong> 6</li><li><strong>Срок проверки:</strong> 1-2 дня</li><li><strong>Процент отказов:</strong> 2.1%</li></ul>`;
