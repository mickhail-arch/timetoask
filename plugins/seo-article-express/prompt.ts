...
const TONE_MAP: Record<string, string> = {
  ...
  medical: `Стиль: профессиональный врач-практик, который объясняет пациенту. Главный принцип: сложное медицинское знание, изложенное простым и тёплым языком. Читатель должен чувствовать, что с ним говорит компетентный специалист, которому можно доверять. Характеристики: - Используй медицинские термины, но ВСЕГДА давай пояснение в скобках или через тире при первом упоминании: «гипертензия (повышенное давление)». - Избегай запугивания. Вместо «это опасно для жизни» — «это сигнал, на который стоит обратить внимание». - Формулируй рекомендации мягко, но уверенно: «я рекомендую», «важно обратить внимание», «стоит обсудить с врачом». - Приводи конкретные цифры и нормы: «нормальный уровень — от 3.5 до 5.5 ммоль/л». - Не ставь диагнозов и не назначай лечение. Всегда направляй к специалисту: «точный диагноз поставит только врач после обследования». - Тон — спокойный, уверенный, без снисходительности. Как разговор с врачом, который уважает пациента. - В конце каждого ключевого раздела можно добавить одно предложение-рекомендацию. - Не используй разговорные сокращения, сленг, восклицательные знаки. === ДОКАЗАТЕЛЬНАЯ МЕДИЦИНА === СТРОГОЕ ПРАВИЛО: все медицинские утверждения должны соответствовать принципам доказательной медицины (Evidence-Based Medicine). - Ссылайся только на факты, подтверждённые мета-анализами, систематическими обзорами, рандомизированными клиническими исследованиями (РКИ) или клиническими рекомендациями Минздрава РФ. - НЕ выдумывай исследования, цифры, названия препаратов, дозировки. Если не уверен в факте — НЕ пиши его. Лучше написать «по данным клинических рекомендаций» без конкретной цифры, чем выдумать цифру. - НЕ приписывай эффективность методам, которые не имеют доказательной базы (гомеопатия, литотерапия, уринотерапия и т.д.). - При упоминании лекарственных препаратов используй МНН (международное непатентованное название), а не торговые марки. Пример: «ибупрофен», а не «Нурофен». - ВСЕГДА добавляй оговорку: «перед применением проконсультируйтесь с врачом». === ЗАКОНОДАТЕЛЬСТВО РФ === СТРОГОЕ ПРАВИЛО: текст должен соответствовать законодательству Российской Федерации в сфере здравоохранения и рекламы. Федеральный закон № 323-ФЗ «Об основах охраны здоровья граждан»: - НЕ ставить диагнозы, НЕ назначать лечение, НЕ рекомендовать конкретные дозировки. - НЕ гарантировать результат лечения: запрещены формулировки «вылечит», «избавит навсегда», «гарантированный эффект». - При любых рекомендациях указывать: «необходима консультация специалиста». Федеральный закон № 38-ФЗ «О рекламе» (ст. 24, 25): - НЕ создавать впечатление, что лекарство безопасно и не требует консультации врача. - НЕ обращаться к несовершеннолетним в рекламе лекарств. - НЕ использовать образ врача для рекламы лекарств (если текст рекламный). - При упоминании любого лекарственного средства обязательно: «Имеются противопоказания. Необходима консультация специалиста». === БАДы (биологически активные добавки) === КРИТИЧЕСКИ ВАЖНО — БАДы НЕ являются лекарственными средствами. При упоминании любого БАД: - ОБЯЗАТЕЛЬНО указывать: «БАД. Не является лекарственным средством». - ЗАПРЕЩЕНО приписывать БАДам лечебные свойства: нельзя «лечит», «исцеляет», «устраняет заболевание». - ЗАПРЕЩЕНО заявлять, что БАД заменяет лекарство или диету. - Допустимые формулировки: «может способствовать», «применяется в качестве дополнительного источника», «используется как дополнение к основному рациону». - Ссылаться на ТР ТС 021/2011 и ФЗ № 29-ФЗ «О качестве и безопасности пищевых продуктов». - При упоминании конкретных веществ (витамин D, омега-3, коллаген и т.д.) — указывать рекомендуемые суточные нормы по СанПиН, не завышать. - НЕ приводить «исследования», доказывающие лечебный эффект БАД, если это не подтверждено Кокрейновскими обзорами или мета-анализами. === ЗАПРЕЩЁННЫЕ ФОРМУЛИРОВКИ === Никогда не используй в тексте: - «гарантирует излечение», «100% результат», «панацея» - «врачи скрывают», «фармкомпании не хотят чтобы вы знали» - «народное средство лучше антибиотиков» - «клинически доказано» (без указания конкретного исследования) - «рекомендуют все врачи», «одобрено ведущими специалистами» (без конкретики) - Любые антивакцинные, антинаучные или конспирологические утверждения`,
  simple:
    'Максимально простой и понятный язык. 6-12 слов. Обращение: «вы».',
};

function buildToneBlock(tone: string, toneComment?: string, gender?: string, ages?: string): string {
  const desc =
    TONE_MAP[tone] ??
  ...
  if (tone === 'medical' && (gender || ages)) {
    block += '\n\nАДАПТАЦИЯ ПОД АУДИТОРИЮ:';
    if (gender === 'female' && ages) {
      if (ages.includes('18') || ages.includes('25') || ages.includes('35')) {
        block += '\nАудитория — молодые женщины, возможно мамы или будущие мамы. Стиль подачи: как от врача, который работает с беременными и молодыми мамами. Мягко, с пониманием тревог. Примеры из практики ведения беременности и детского здоровья. Без менторского тона — как доверительная беседа с доктором, который понимает нагрузку материнства.';
      } else if (ages.includes('45') || ages.includes('55') || ages.includes('65')) {
        block += '\nАудитория — женщины зрелого возраста. Стиль подачи: уважительный, без покровительства. Фокус на профилактике, качестве жизни, гормональных изменениях. Как от врача-терапевта с 20-летним стажем.';
      }
    } else if (gender === 'male') {
      block += '\nАудитория — мужчины. Стиль подачи: прямой, конкретный, без лишних эмоций. Факты, цифры, чёткие рекомендации. Как от врача-хирурга: коротко, по делу, с уважением к времени пациента.';
    } else if (gender === 'any' || !gender) {
      block += '\nАудитория — широкая, смешанная. Стиль подачи: нейтральный, универсальный. Без гендерной специфики. Как от семейного врача.';
    }
  }
...
function buildCtaBlock(
  ...
  const type = ctaType ?? 'service';
  const style = ctaStyle ?? 'standard';
  const position = ctaPosition ?? 'end';

  const typeDesc = type === 'product'
    ? 'CTA направлен на покупку товара. Подводи читателя к мысли, что этот товар решает его проблему. Используй формулировки: «выбрать подходящий вариант», «посмотреть каталог», «сравнить модели». Не давить — показать выгоду владения.'
    : 'CTA направлен на заказ услуги. Подводи читателя к мысли, что самостоятельно решать проблему сложно/долго, а профессионал сделает лучше. Формулировки: «получить консультацию», «обсудить задачу», «узнать подробности».';

  const styleDesc = style === 'native'
    ? `Нативная встройка: CTA должен быть ОРГАНИЧНО вплетён в текст, как естественное продолжение мысли абзаца. НЕ выделять отдельным блоком, НЕ использовать <blockquote>, НЕ использовать «Кстати,» или «К слову,». Просто одно предложение в конце абзаца с ссылкой, которое читается как часть рассуждения. Читатель не должен чувствовать рекламный посыл.`
    : `Стандартная встройка: CTA оформляется как отдельный визуально выделенный блок — <blockquote> с призывом к действию и ссылкой. Это явный рекламный блок, читатель понимает что это предложение.`;

  let positionDesc = '';
  if (position === 'start') {
    positionDesc = 'Размести CTA ОДИН раз — в первом H2-разделе (после 2-3 абзацев, не в самом начале). Больше нигде CTA не ставить.';
  } else if (position === 'middle') {
    positionDesc = 'Размести CTA ОДИН раз — в середине статьи (в 3-м или 4-м H2-разделе). Больше нигде CTA не ставить.';
  } else if (position === 'end') {
    positionDesc = 'Размести CTA ОДИН раз — в последнем H2-разделе перед заключением. Больше нигде CTA не ставить.';
  } else if (position === 'all') {
    positionDesc = 'Размести CTA ТРИ раза: первый — в конце 2-го H2-раздела, второй — в середине статьи, третий — перед заключением. Каждый раз ПЕРЕФРАЗИРУЙ текст CTA, не повторяй дословно. Между двумя CTA-блоками минимум 2000 символов.';
  }

  const block = `=== ПРИЗЫВ К ДЕЙСТВИЮ (CTA) ===
Текст CTA: "${cta}"
${ctaUrl ? `Ссылка: ${ctaUrl}` : 'Без ссылки.'}
Формат ссылки: <a href="${ctaUrl}" target="_blank">${cta}</a>
...
- CTA не должен стоять в том же абзаце где стоит ссылка перелинковки или ссылка на источник. Минимум 500 символов между CTA и любой другой ссылкой.
- CTA не ставить в блоке FAQ, «Кратко», введении, заключении (кроме position=end — тогда можно в последнем H2 перед заключением).
- CTA не ставить рядом с блоком сравнения или кейсом — минимум 1 абзац отступа.
- При нативном стиле CTA-ссылка учитывается в общем лимите ссылок (1 на 2000 символов).
- При стандартном стиле CTA-блок НЕ учитывается в лимите ссылок (он вне основного текста).`;
...
export function buildSystemPrompt(
  ...
  // 1.1. Контекстные ограничения от автора — высший приоритет
  if (contextNotes) {
    blocks.push(`=== КРИТИЧЕСКИ ВАЖНЫЕ ОГРАНИЧЕНИЯ ОТ АВТОРА ===
Автор статьи указал следующие смысловые ограничения и контекст:

"${contextNotes}"

ЭТИ УКАЗАНИЯ ИМЕЮТ АБСОЛЮТНЫЙ ПРИОРИТЕТ:
- Темы, которые автор просит НЕ затрагивать — не упоминай вообще, ни в одном разделе, ни в FAQ.
- Аудитория или сегмент, указанные автором (B2C / B2B / новички / профи / конкретный регион / возрастная группа) — пиши строго для них. Если автор сказал "только B2C" — никаких упоминаний оптовых продаж, маркетплейсов для бизнеса, B2B-кейсов.
- Конкретный контекст или ракурс, указанный автором — соблюдай его в каждом H2-разделе.
- Если автор указал что-то конкретное (бренды, страны, методы), на которые нужно ссылаться или НЕ ссылаться — следуй этому.

Если какой-либо H2-раздел из ТЗ противоречит этим указаниям — переформулируй содержание этого раздела под ограничения автора, но НЕ нарушай их. Если бриф предполагает обсуждение темы X, а автор просит её не затрагивать — этот H2 пиши о смежном аспекте, который не нарушает ограничение.

Эти указания важнее Tone of Voice, важнее общих рекомендаций по структуре, важнее любых SEO-практик. Если возникает конфликт — следуй указаниям автора.`);
  }
...
${cta ? '10. CTA (после FAQ, отдельный абзац, не считается в объём)' : ''}`);
  ...
  blocks.push(buildToneBlock(tone, toneComment || undefined, gender, ages.join(',')));
...
Без воды и повторения того что уже сказано.`);
  ...
  if (cta) {
    const ctaType = (input.cta_type as string) ?? undefined;
    const ctaStyle = (input.cta_style as string) ?? 'standard';
    const ctaPosition = (input.cta_position as string) ?? 'end';
    blocks.push(buildCtaBlock(cta, intent, ctaUrl ?? '', ctaType, ctaStyle, ctaPosition));
  }
