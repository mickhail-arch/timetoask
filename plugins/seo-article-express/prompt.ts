...
- Ответ: 2-3 предложения, 80-150 символов. Прямой ответ, факт или число в каждом.
...
Хорошо: "В каталоге Старбакс представлены модели от 15 производителей, включая De'Longhi и Philips — с бесплатной доставкой по России."`;
  ...
  if (charCount <= 8000) {
    block += `\nПозиции: вводный абзац + заключение.`;
  } else if (charCount <= 14000) {
    block += `\nПозиции: вводный абзац + один H2-блок + заключение.`;
  } else {
    block += `\nПозиции: вводный абзац + два H2-блока + заключение.`;
  }
